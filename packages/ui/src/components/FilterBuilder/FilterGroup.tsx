      ...
      {group.rules.length > 0 && (
        <div className={cn(groupChildren())}>
          {group.rules.map((child, index) => {
            ...
            // Ключ по позиции — почему index-ключи и их единственный компромисс
            // см. в примечании под компонентом.
            if (isGroup(child)) {
              return (
                <FilterGroup
                  key={index}
...
// Рекурсивная панель группы в React.memo: правка, изолированная в одной ветке
// (за счёт структурного шаринга дерева), не перерисовывает несвязанные группы.
//
// Позиционное ключевание (принятое ограничение): дети ключуются по index — в
// нормализованной модели нет id узла, идентичность узла и есть его позиция в rules
// родителя. Альтернативы негодны: WeakMap<node,id> ремонтировал бы инпут на каждый
// keystroke (правка заменяет правило новым объектом, ключ меняется, фокус слетает),
// а явный id — это смена модели и wire-формата, которую слой намеренно отверг.
// Компромисс: при удалении не-последнего сиблинга React переиспользует DOM по index,
// поэтому переходное состояние (фокус, каретка, IME) может остаться на инпуте, теперь
// принадлежащем следующему правилу; controlled-значения всегда переспроецируются верно.
