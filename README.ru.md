...
Тем, кто уже на Tailwind, нужен тот же импорт: в Tailwind 4 нет формата JS-пресета, поэтому `@sergeyhorse/forge/preset` отдаёт значения токенов обычными объектами для инструментов, а не как конфиг.
...
- **FilterBuilder, который переживает URL.** Полностью контролируемое дерево вложенных AND/OR-групп со schema-driven редакторами для string, number, boolean, date и enum. Сериализуется в версионированный конверт, который на обратном пути валидируется по ключам каждого узла: фильтр, вставленный из адресной строки, либо разбирается в нормализованное дерево, либо падает с путём до битого узла. Наполовину разобранный фильтр в состояние не попадёт. Правка одного правила перерисовывает только это правило, и на это есть тест.
...
Headless-слой тоже экспортируется: `useDataGrid`, `useCombobox`, операции над деревом FilterBuilder и его сериализатор. Берите их, если нужна логика без нашей разметки.
...
Релизы автоматические: пуш в `main` с непогашенным changeset поднимает версию, публикует пакет в npm и возвращает тег в репозиторий.
